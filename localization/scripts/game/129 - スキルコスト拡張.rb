...
class Game_Actor
  ...
  def skill_cost_payable?(skill)
    ...
    tmskcost_game_actor_skill_cost_payable?(skill) &&
      (!item || ($game_party.has_item?(item)&&($game_party.item_number(item)>=skill.item_number_a))) &&
  ...
  def pay_skill_cost(skill)
    ...
    item = $data_items[skill.item_cost]
    item_number = skill.item_number_a
    $game_party.lose_item(item, item_number) if item
