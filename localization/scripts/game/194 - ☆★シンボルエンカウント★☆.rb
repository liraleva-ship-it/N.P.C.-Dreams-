    ...
    3 => [
        ...
        7 => [
         ...
         8 => [
          ...
          2, # プレイヤーレベルの判断種別
          5, # シンボルが反応するプレイヤーとの距離
          7, # シンボルが反応するプレイヤーとの距離（プレイヤーダッシュ時）
          0, # プレイヤーに反応していない時の移動パターン
          0, # 透明度の変化するプレイヤーとの距離
          4, # 反応前の移動速度
          6, # 反応後の移動速度
...
class Game_Event < Game_Character
  ...
  if $game_switches[995] == true && $game_switches[962] == true 
    data = SYMBOL_SETTING_LIST[id]
    @symbol_encount = true
    @symbol_away_level = data[0]
    @symbol_away_level_type = data[1]
  ...
  def passable?(x, y, d)
    if @symbol_encount
      ...
      return false if @unpassable_region.include?($game_map.region_id(x2, y2)) && $game_switches[995] == false
    end
    super
  end
  #--------------------------------------------------------------------------
  ...
  def active_symbol_encount?
    if $game_map.interpreter.running?
      ...
      @forming = false
      false
    elsif $game_switches[995] == true && $game_switches[962] == false
      false
    else
      true
    end
  ...
  #--------------------------------------------------------------------------
  def start_forming
#~     if $game_map.map_id == current_floor
...
#~     end
  ...
  def update_symbol_reaction
    if active_symbol_encount?
      if !@forming && reaction? # $game_player.current_floor == $game_map.layer_index.index(current_floor)
        start_forming
