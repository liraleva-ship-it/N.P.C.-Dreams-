...
module ConvertAttack
  ...
  Word = "Conversion Attacks"  
  
  #変換攻撃特徴設定用キーワードを指定。
  
  Words = ["HP Conversion"  ,"MP Conversion"  ,"TP Conversion"  ]
  ...
  MA[0] = {
  :hp => "%s's %s decreased by %s!"  ,
  :mp => "%s's %s decreased by %s!"  ,
  :tp => "%s's %s decreased by %s!"  }
  
  #アクターHP/MP/TP回復時メッセージ
  
  MA[1] = {
  :hp => "%s devoured %s's %s!"  ,
  :mp => "%s's %s recovered by %s!"  ,
  :tp => "%s's %s increased by %s!"  }
...
#~             $game_variables[24] = rand(3)
#~     $game_variables[22] = 10 if $game_variables[24] == 0
#~     $game_variables[23] = 70 if $game_variables[24] == 0
#~     $game_variables[22] = 20 if $game_variables[24] == 1
#~     $game_variables[23] = 20 if $game_variables[24] == 1
#~     $game_variables[22] = 30 if $game_variables[24] == 2
#~     $game_variables[23] = 5 if $game_variables[24] == 2
#~     case rand(3)
#~     when 0; $game_variables[22] = 7; $game_variables[23] = 70
#~     when 1; $game_variables[22] = 15; $game_variables[23] = 20
#~     when 2; $game_variables[22] = 30; $game_variables[23] = 5
#~     end
    a = user
    b = self
    v = $game_variables
...
#~       if user.state?(42)
#~         @result.exhp_convert = 0
#~       end
#~       if user.state?(43)
#~         @result.exhp_convert /= 2
#~       end
