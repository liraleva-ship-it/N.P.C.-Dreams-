...
class Game_Battler < Game_BattlerBase
  ...
  def regenerate_all
    if alive?
      ...
      if (hp_damage != 0 or mp_damage != 0 or tp_damage != 0) and
        ...
        p ["Regen per Action"  , name, "HP Regen"  , hp_damage, "MP Regen"  , mp_damage, "TP Regen"  , tp_damage]
  ...
  def state_frame_update
    ...
    if flag == true
      ...
      p ["Regen per Turn"  , name, "HP Regen"  , hp_change, "MP Regen"  , mp_change, "TP Regen"  , tp_change]
  ...
  def state_regenerate_message_set(id)
    case id
    when 0; return  "%s's HP recovered by %s!"  
    when 1; return  "%s received %s HP damage!"  
    when 2; return  "%s's MP recovered by %s!"  
    when 3; return  "%s received %s MP damage!"  
    when 4; return  "%s's TP recovered by %s!"  
    when 5; return  "%s received %s TP damage!"  
    end
  end
end
#==============================================================================
